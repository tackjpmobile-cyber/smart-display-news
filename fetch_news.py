...
def fetch_article_body(url, max_chars=BODY_CHARS, min_chars=80, gap_threshold=400):
    """記事リンク先から本文らしきテキストを抜き出し、冒頭max_chars文字を返す。
    取得・解析に失敗した場合、または抽出結果がmin_chars未満(=本文として不十分)の場合は
    空文字を返す(呼び出し側でスニペット/タイトルにフォールバックする)。"""
    try:
    ...
    # script/style等を除去(ナビ・注意書き等のノイズを減らす)
    cleaned = re.sub(r"(?is)<(script|style|noscript|header|footer|nav)[^>]*>.*?</\1>", " ", page_html)

    # 本文は<p>タグが連続して密集している箇所に入っていることが多い。
    # 単純に全<p>を先頭から集めると、案内文や関連記事リストのような
    # 散発的な短い段落を本文として誤採用しやすいため、
    # 「間隔の狭い<p>のかたまり」ごとにグループ化し、
    # 最も文字量の多いかたまりを本文とみなす。
    matches = list(re.finditer(r"(?is)<p[^>]*>(.*?)</p>", cleaned))
    text = ""
    if matches:
        clusters = [[matches[0]]]
        for m in matches[1:]:
            if m.start() - clusters[-1][-1].end() <= gap_threshold:
                clusters[-1].append(m)
            else:
                clusters.append([m])

        def cluster_text(cluster):
            return " ".join(strip_html(m.group(1)) for m in cluster).strip()

        best_cluster = max(clusters, key=lambda c: len(cluster_text(c)))
        text = cluster_text(best_cluster)

    # <p>タグからの抽出が短すぎる(構造が特殊なサイト等)場合は、ページ全体からの抽出を試す
    if len(text) < min_chars:
        whole = strip_html(cleaned)
...
def parse_items(xml_bytes, keyword=""):
    ...
    for item in channel_items:
        ...
        resolved_link = resolve_link(raw_link)
        if not source:
            source = domain_from_url(resolved_link)
        items.append({
            "title": title,
            "link": resolved_link,
            "source": source,
